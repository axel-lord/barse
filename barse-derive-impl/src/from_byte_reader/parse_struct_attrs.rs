...
pub fn parse_struct_attrs(attrs: &[Attribute], ctx: &Ctx) -> Result<StructAttrs, TokenStream> {
    ...
    for item in parse_attrs::parse_attrs(attrs, ctx) {
        let item: syn::MetaNameValue =
            syn::parse2(item?.to_token_stream()).map_err(syn::Error::into_compile_error)?;

        let lit_str: syn::LitStr =
            syn::parse2(item.value.to_token_stream()).map_err(syn::Error::into_compile_error)?;

        if let Some(ident) = item.path.get_ident() {
            if ident == &ctx.error_attr {
                let ty = lit_str.parse().map_err(syn::Error::into_compile_error)?;
            ...
            if ident == &ctx.from_attr {
                let path = lit_str.parse().map_err(syn::Error::into_compile_error)?;
